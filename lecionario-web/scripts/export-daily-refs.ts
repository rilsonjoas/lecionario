/** Gera a tabela `{data ISO: referências do dia}` consumida pelo Bíblia
 *  na Arte pra ligar a "Pintura do Dia" à leitura litúrgica real (ver
 *  `biblia-na-arte/server/src/lib/lectionary-refs.ts` e ROADMAP
 *  "Pintura do Dia sumindo em alguns dias + sincronizar..." 2026-09-02).
 *
 *  Roda a lógica REAL de calendário litúrgico deste repo
 *  (`getLiturgicalCycle` + `getRCLReadings`, que já lida com Páscoa
 *  móvel/ciclo A-B-C por ano) em vez de reimplementar essa conta do
 *  lado do Bíblia na Arte — só o RESULTADO é copiado pra lá.
 *
...
import { getLiturgicalCycle } from '../src/lib/liturgical-calendar';
...
const readingsByDate: Record<string, string[]> = {};
...
while (cur <= END) {
  ...
  if (result && result.readings.length > 0) {
    readingsByDate[toISO(cur)] = [...new Set(result.readings.map((r) => r.reference))];
  }
  cur = addDays(cur, 1);
}
