...
- **Header institucional (web)** — em /apoiar e /privacidade o logo "Lecionário" agora é link para a home, e a coluna de data/info litúrgica ("sábado, 22 de agosto · Sábado do Tempo Comum · Ano A") some — ruído onde não agrega. Nova variante `minimal` do Header mantém marca + botão de tema. No modo escuro, o Dialog do glossário ganhou superfície `card` sólida com sombra (antes sumia no fundo) e o botão HOJE virou pill com borda; ModeToggle vestido na paleta do header.
...
- **"Voltar para hoje" sai do miolo da barra de navegação (web)** — o link poluía a coluna da data; agora mora no cluster direito depois de Próximo Dia, separado por divisor fino, e continua desaparecendo quando já é hoje.
- **Cópias com expansão e assinatura padrão** — Citação do dia ganhou botão de copiar nos dois apps (critério: texto que circula em redes merece cópia); TODAS as cópias agora terminam com a assinatura "— Lecionário · lecionario.narniano.com" (o share do dia migrou do antigo "— Lecionário App"). Critério permanente registrado no ROADMAP: um botão por card, só onde o usuário vai colar.
- **Modo escuro v2: fundo sazonal como no mobile** — primeira versão tinha virado carvão neutro e matava dois patrimônios: a identidade da estação e a legibilidade dos títulos (`text-secondary` afundando). Agora `--background` no escuro é a **cor da estação escurecida** via `color-mix` com carvão (verde-sálvia escuro no Tempo Comum, vinho profundo na Quaresma... exatamente como a home do mobile mantém o primaryColor), cards continuam em superfície neutra escura, `--secondary` vira texto claro (títulos/citações legíveis) e `muted-foreground` sobe de luminosidade.
