// Espelha lecionario-web/src/lib/quote-fetcher.ts (mesma API) — este
// projeto não usa pacote compartilhado entre mobile e web, então a
// duplicação é intencional (ver artwork-fetcher.ts como precedente).
//
...
export async function fetchDailyQuote(signal?: AbortSignal): Promise<DailyQuote | null> {
  try {
    ...
    const data = (await res.json()) as Omit<DailyQuote, 'quote'> & { text: string };
    return { ...data, quote: data.text };
  } catch {
    return null;
  }
}
